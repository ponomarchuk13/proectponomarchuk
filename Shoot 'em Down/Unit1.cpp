...
//---------------------------------------------------------------------------
//===========================================================================
void pause_game(AnsiString text) // now you can make some tea between waves
{
    if (!on_pause)
    {
        //AnsiString text = "GAME PAUSED";
...
{
    if (Key = 27) pause_game("GAME PAUSED");
}
//===========================================================================
//---------------------------------------------------------------------------
void __fastcall TForm1::TIMETimer(TObject *Sender)
{
    ...
    if (!Form1->Active) pause_game("GAME PAUSED"); // you need a break
...
void __fastcall TForm1::img_golden_duckClick(TObject *Sender)
{
    if (duck[9].can_hit)
    {
        ...
        if (ammo>0)
        {
            ...
            change_image(duck_img[9].image, Form1->duck_list, duck[9].dirX+4);
            //Form1->lable_kill_counter->Caption = "Ducks killed -- "+IntToStr(kill_count);
        }
    }
}
...
void duck_quickmove(TImage *img, int step, bool &need_move, duck_stat *duck, int id)
{
    ...
    else change_image(duck_img[9].image, Form1->duck_list, duck[9].dirX+4);

    if (id == 9) // gold duck flies faster than other ducks in 1.5 times
    {
        step*=1.5;
    }
...
{
    kill_count = 0;
    pause_game("YOU ARE OUT OF AMMO");
    Default(duck);
    ammo = level[current_level].ammo;
    gold_duck_count = level[current_level].gold_duck_count;
}
